  ...
  <div class="bg-white flex flex-col gap-x-3 rounded-md shadow-sm">
    <div class="p-3">
      ...
      <div class="flex items-center justify-center mt-2 gap-x-1">
        <button @click="likeItem" class="like-btn group" :class="{ 'like-item-active': allReadyLike }">
        ...
        <button @click="bookMark" class="bookmark-btn group" :class="{ 'bookmark-item-active': allReadyBookMark }">
  ...
  methods: {
    likeItem() {
      ...
      if (!this.allReadyLike) {
        // store'daki totel ekliyoruz sürekli, burdada match işlemi yapılıyor
        likes = [...likes, this.item.id];
      } else {
        likes = likes.filter((x) => x != this.item.id);
      }
    ...
    bookMark() {
      ...
      if (!this.allReadyBookMark) {
        // store'daki totel ekliyoruz sürekli, burdada match işlemi yapılıyor
        bookmarks = [...bookmarks, this.item.id];
      } else {
        bookmarks = bookmarks.filter((x) => x != this.item.id);
      }
